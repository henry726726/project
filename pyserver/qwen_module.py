...
def enforce_text_rules(items, subject_bbox, min_ar=1.8, min_margin=0.03, max_iou=0.2):
    ...
    for it in items:
        ...
        b = clip_bbox(b)
        if subject_bbox and iou(b, subject_bbox) >= max_iou: continue
        x, y, w, h = b
        if x < min_margin or y < min_margin or x + w > 1 - min_margin or y + h > 1 - min_margin: continue
...
def postprocess_layout(parsed, text_iou_thr=0.3, logo_iou_thr=0.3, subj_text_iou_max=0.2):
    if not isinstance(parsed, dict) or "layout" not in parsed: return parsed
    ...
    for i, t in enumerate(texts): t.setdefault("id", f"text#{i}")
    ...
    for g in graphics:
        ...
        if not (isinstance(b, list) and len(b) == 4): continue
        b = clip_bbox(b)
        if iou(b, subject_bbox) > 0.4: continue
        g["bbox"] = b
...
# -------------------------------------------------
def normalize_if_pixels_layout(parsed, image_size):
    try:
        W, H = image_size
    except Exception:
        W, H = 1, 1
...
# -------------------------------------------------
def extract_palette_hex(image_data, k=5):
    try:
...
def ensure_background_prompts(parsed, product_name, context_json, min_chars=800):
    ...
    if prompt_raw.startswith('{') or '"background_prompt"' in prompt_raw:
        ...
        if fields.get("background_prompt"): bg["prompt"] = fields["background_prompt"]
        if fields.get("negative_prompt") and not negative: negative = fields["negative_prompt"]
        cam = bg.get("camera") or {}; cam.update(fields.get("camera") or {}); bg["camera"] = cam
        light = bg.get("lighting") or {}; light.update(fields.get("lighting") or {}); bg["lighting"] = light
    prompt = (bg.get("prompt") or '').strip()
    light_raw = bg.get("lighting", {})
    if isinstance(light_raw, dict): light = light_raw
    elif isinstance(light_raw, str) and light_raw.strip(): light = {"type": light_raw.strip()}
    else: light = {}
    kor2eng_light = {
        "균일한 밝기": "soft", "부드러운": "soft", "딱딱한": "hard", "림": "rim", "주변광": "ambient",
        "상단": "top", "하단": "bottom", "앞": "front", "뒤": "back", "왼쪽": "left", "오른쪽": "right",
    }
    if "type" in light: light["type"] = kor2eng_light.get(light["type"], light["type"]) or "soft"
    if "direction" in light: light["direction"] = kor2eng_light.get(light["direction"], light["direction"]) or "front"
    cam_raw = bg.get("camera", {}); cam = cam_raw if isinstance(cam_raw, dict) else {}
    ...
    if len(prompt) < min_chars:
        ...
        bg["prompt"] = prompt
    if not negative: bg["negative_prompt"] = _build_negative_prompt_default()
    else: bg["negative_prompt"] = negative
    cam["angle"] = angle; cam["distance"] = distance; light["type"] = ltype; light["direction"] = ldir
...
# -------------------------------------------------
async def generate_bg_plan(model, processor, image_data, product_name, parsed, palette,
    ...
    messages = [
        ...
        {"role": "user", "content": [
            {"type": "image", "image": f"data:image/jpeg;base64,{base64.b64encode(image_data).decode('utf-8')}"},
...
class QwenProcessor:
    """
    Qwen VLM 모델을 사용하여 이미지 분석 및 배경 프롬프트를 생성하는 클래스.
    """
    ...
    async def process_image(self, image_data: bytes, product_name: str, bg_prompt_enabled: bool = True):
        """
        주어진 이미지와 제품명을 바탕으로 Qwen 모델을 사용하여 분석을 수행합니다.
        
        Args:
            image_data (bytes): Base64로 디코딩된 원본 이미지 데이터 (바이트).
            product_name (str): 제품명.
            bg_prompt_enabled (bool): 배경 프롬프트 생성 여부.
        ...
        try:
            ...
            parsed = normalize_if_pixels_layout(parsed, image_size)
            parsed = postprocess_layout(parsed)
            parsed = inject_fallback_boxes(parsed)
            parsed = add_text_underlays(parsed)

            # Second-pass
            if bg_prompt_enabled:
                palette = extract_palette_hex(image_data, k=5)
                bg_plan = await generate_bg_plan(
                    self.model, self.processor, image_data, product_name, parsed, palette,
                    max_new_tokens=1000, top_p=0.9, temperature=0.7
                )
            ...
            parsed = ensure_background_prompts(parsed, product_name, context_for_bg, min_chars=900)
