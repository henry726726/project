...
public class MetaAdCreatorService {
    ...
    public void createInitialAd(MetaAdCreationRequest request) {
        ...
        if (request.getContentId() != null) {
            ...
            content = adContentRepository.findFirstByUserOrderByCreatedAtDesc(user)
        ...
        AdAccount adAccount = adAccountRepository
                .findByAccountIdAndPageId(adAccountId, pageId)
                .orElseThrow(() -> new RuntimeException("광고 계정이 DB에 존재하지 않습니다"));
        ...
        if (request.getStartTime() != null && !request.getStartTime().isBlank()) {
            try {
                LocalDateTime ldt = LocalDateTime.parse(request.getStartTime()); // "2025-08-25T15:30 임의 고정 시간 사용"
    ...
    private Map<String, String> fetchCreativeMeta(String creativeId, String accessToken) {
        String url = "https://graph.facebook.com/v22.0/" + creativeId +
                "?fields=object_story_spec&access_token=" + accessToken;

        ResponseEntity<String> response = restTemplate.getForEntity(url, String.class);
        if (!response.getStatusCode().is2xxSuccessful()) {
            throw new RuntimeException("❌ Creative 조회 실패: " + response.getBody());
        }

        try {
            JsonNode json = objectMapper.readTree(response.getBody());
            JsonNode linkData = json.path("object_story_spec").path("link_data");

            Map<String, String> result = new HashMap<>();
            result.put("headline", linkData.path("name").asText(""));
            result.put("caption", linkData.path("caption").asText(""));
            result.put("description", linkData.path("description").asText(""));
            return result;
        } catch (Exception e) {
            throw new RuntimeException("❌ Creative 파싱 실패: " + response.getBody(), e);
        }
    }

    private String createAdCreative(
            String adAccountId,
            String accessToken,
            String message,
            String imageHash,
        ...
        String objectStorySpecJson = String.format(
                ...
                imageHash,
                escapeForJson(link),
                escapeForJson(headline != null ? headline : ""),
                escapeForJson(caption != null ? caption : ""),
                escapeForJson(description != null ? description : ""));

        MultiValueMap<String, String> body = new LinkedMultiValueMap<>();
        body.add("name", "Updated Creative");
        body.add("object_story_spec", objectStorySpecJson);
        body.add("access_token", accessToken);

        return postAndExtractId("https://graph.facebook.com/v22.0/" + adAccountId + "/adcreatives", body);
    }
    ...
    public void updateAd(Long adRunId, Long newContentId, String userEmail, String newText, String newImageBase64) {
        ...
        // ✅ 기존 Creative에서 headline/caption/description 추출
        Map<String, String> creativeMeta = fetchCreativeMeta(adRun.getCreativeId(), accessToken);

        // 새 이미지 업로드
        String newImageHash = uploadImageToFacebook(adRun.getAccountId(), accessToken,
                newContent.getGeneratedImageBase64());

        // 새 Creative 생성 (headline/caption/description 그대로 재사용)
        String newCreativeId = createAdCreative(
                adRun.getAccountId(),
                accessToken,
                newContent.getAdText(), // message
                newImageHash,
                adRun.getPageId(),
                adRun.getLink(),
                creativeMeta.get("headline"),
                creativeMeta.get("caption"),
                creativeMeta.get("description"));
        ...
        adRun.setAdModifiedAt(OffsetDateTime.now(ZoneOffset.UTC));
