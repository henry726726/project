...
public class ReportService {
    ...
    @Autowired
    // private AdAccountRepository adAccountRepository; // 이 서비스에서는 직접 사용하지 않으나, 빈
    // 주입은 그대로 유지 (주석 처리)
    ...
    public byte[] generateAdPerformancePdfBytes() throws IOException {
        ...
        try {
            ...
            baseFont = BaseFont.createFont(new ClassPathResource(RESOURCE_PATH_NANUM_GOTHIC).getPath(),
        ...
        try {
            ...
            document.add(new Paragraph("생성일: " + LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")),
                    contentFont));
            document.add(new Paragraph("\n"));

            // ⭐️ 여기부터 데이터 조회 로직을 수정합니다.
            // 이제 특정 adId에 해당하는 Ad를 하나만 가져옵니다.
            Optional<Ad> fixedAdOpt = adRepository.findByAdId(FIXED_REPORT_AD_ID); // AdRepository에 findByAdId 메서드가
                                                                                   // 필요합니다.
            ...
            List<AdInsight> insightsForFixedAd = adInsightRepository.findAllByAdIdAndDate(fixedAd.getAdId(),
                    FIXED_REPORT_DATE);

            if (insightsForFixedAd.isEmpty()) {
                document.add(new Paragraph(
                        "고정된 광고(" + FIXED_REPORT_AD_ID + ")에 대한 " + FIXED_REPORT_DATE + " 날짜의 성과 데이터를 찾을 수 없습니다.",
                        contentFont));
                document.close();
            ...
            document.add(new Paragraph("상태: " + (fixedAd.getStatus() != null ? fixedAd.getStatus() : "알 수 없음"),
                    contentFont));
            document.add(new Paragraph("\n"));
            ...
            if (adContentOpt.isPresent()) {
                ...
                if (adContent.getOriginalImageBase64() != null && !adContent.getOriginalImageBase64().isEmpty()) {
                    try {
                        Image img = null;
                        String imageUrl = adContent.getOriginalImageBase64().trim();

                        // Base64 인코딩된 이미지인지 URL 이미지인지 판단
                        boolean isBase64Encoded = imageUrl.startsWith("data:image/")
                                || (imageUrl.length() > 200 && !imageUrl.contains("://"));
                        ...
                        System.err.println("❌ 광고 이미지 로드 또는 디코딩 실패 (데이터 시작: "
                                + adContent.getOriginalImageBase64().substring(0,
                                        Math.min(adContent.getOriginalImageBase64().length(), 100))
                                + "...): " + e.getMessage());
            ...
            // (이미 특정 날짜의 데이터만 가져왔으므로, 리스트 내에서는 단일 광고/날짜에 대한 여러 연령/성별 Breakdown 데이터가 있을 수
            // 있음)
            ...
            if (representativeInsight != null) {
                ...
                addTableCell(summaryTable, String.valueOf(representativeInsight.getImpressions()), tableCellFont,
                        false);
                addTableCell(summaryTable, String.valueOf(representativeInsight.getClicks()), tableCellFont, false);
                addTableCell(summaryTable, String.format("₩%.2f", representativeInsight.getSpend()), tableCellFont,
                        false);
                addTableCell(summaryTable, String.format("%.2f%%", representativeInsight.getCtr()), tableCellFont,
                        false);
                ...
                for (AdInsight insight : insightsForFixedAd) {
                    addTableCell(breakdownTable, insight.getAge() != null ? insight.getAge() : "알 수 없음", tableCellFont,
                            false);
                    addTableCell(breakdownTable,
                            insight.getGender() != null
                                    ? (insight.getGender().equals("female") ? "여성"
                                            : (insight.getGender().equals("male") ? "남성" : "알 수 없음"))
                                    : "알 수 없음",
                            tableCellFont, false);
