...
import './MyPage.css'
import { Link } from 'react-router-dom'; // Link는 Header/Footer에서 사용되므로 유지합니다.
...
// Header는 다른 파일로 분리하고 MyPage에서는 import하여 사용하는 것이 일반적인 구조입니다.
...
// ===================== MyPage 컴포넌트 =====================
const MyPage = ({ userData, onLogout }) => { // onLogout prop은 MyPage 상위 컴포넌트에서 전달
    // const navigate = useNavigate(); // 'navigate'는 이 컴포넌트 내에서 더 이상 사용되지 않으므로 제거합니다.
    ...
    const handleAutoLogout = useCallback(async () => {
        try {
            ...
            console.error("Auto logout failed:", error); // 디버깅을 위해 에러 출력
            // ignore
        }
        onLogout(); // onLogout은 prop으로 넘어오므로 의존성 배열에 추가
        alert('You have been logged out due to inactivity.');
        window.location.reload();
    }, [onLogout]); // onLogout이 이 함수 내부에서 사용되므로 의존성 배열에 추가
    ...
    }, []); // 이펙트 내부에서 사용되는 외부 변수(props, state)가 없으므로 빈 배열 유지
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setIsEditing(false);
            alert('프로필이 성공적으로 업데이트되었습니다.'); // 성공 메시지 추가
        } catch (error) {
            console.error('Save error:', error); // 디버깅을 위해 에러 출력
    ...
    const handleCancel = () => {
        setIsEditing(false);
        setEditInfo(userInfo); // 취소 시 원래 userInfo로 되돌림
        setError('');
    };
    ...
    const handleLogout = async () => {
        try {
            ...
            onLogout(); // 실패해도 일단 로그아웃 처리
            alert('로그아웃 중 오류가 발생했지만, 세션을 종료합니다.'); // 에러 발생 시 메시지
            window.location.reload();
        }
    };
    ...
    const handleGeneratePdf = async () => {
        ...
        try {
            ...
            link.remove();
            */
            alert('PDF 리포트가 성공적으로 생성되었습니다!'); // 성공 메시지 추가
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
            {/* 헤더 컴포넌트 */}
            <Header isLoggedIn={isLoggedIn} onLogout={handleLogout} /> {/* onLogout은 handleLogout으로 연결 */}

            {/* 메인 콘텐츠 영역 (Flexbox로 남은 공간 차지) */}
            <main className='mypage-container' style={{ flexGrow: 1, padding: '20px', fontFamily: "'Noto Sans KR', sans-serif", backgroundColor: '#2a1a4a', color: '#e0e0ff' }}>
                <div className="mypage-header">
                    <div className="mypage-title">My Profile</div>
                    <div className="mypage-underline"></div>
                    <div style={{marginTop: 10, color: '#A8E6CF', fontWeight: 600, fontSize: 16}}>
                ...
                <div className="profile-section">
                    <div className="profile-avatar">
                        <img src={user_icon} alt="Profile Avatar" style={{ width: 100, height: 100, borderRadius: '50%', border: '2px solid #A8E6CF' }} />
                    </div>
                        ...
                        {!isEditing ? (
                            <div className="info-display">
                                <div className="info-item">
                                    <img src={user_icon} alt="User Icon" style={{width: 20, height: 20, marginRight: 5}} />
                                    <span className="label">닉네임:</span>
                                    <span className="value">{userInfo.nickname}</span>
                                </div>
                                <div className="info-item">
                                    <img src={email_icon} alt="Email Icon" style={{width: 20, height: 20, marginRight: 5}} />
                            ...
                            <div className="info-edit">
                                <div className="edit-item">
                                    ...
                                    <input 
                                        ...
                                        disabled={loading}
                                        style={{flex: 1, padding: 8, borderRadius: 5, border: '1px solid #7c4dff', backgroundColor: 'rgba(255,255,255,0.1)', color: '#e0e0ff'}}
                                    />
                                </div>
                                <div className="edit-item">
                                    <img src={email_icon} alt="Email Icon" style={{width: 20, height: 20, marginRight: 5}} />
                                ...
                                <div className="edit-item" style={{display: 'flex', alignItems: 'flex-start'}}>
                                    {/* 이메일 아이콘 대신 textarea에 적절한 아이콘을 넣거나 아이콘을 제거하고 text만 사용할 수 있습니다. */}
                                    <span style={{marginRight: 5, color: '#b3e1e9', paddingTop: 8}}>소개:</span> 
                                    <textarea 
                                        value={editInfo.bio}
                                        ...
                                        rows="3"
                                        disabled={loading}
                                        style={{flex: 1, padding: 8, borderRadius: 5, border: '1px solid #7c4dff', backgroundColor: 'rgba(255,255,255,0.1)', color: '#e0e0ff', minHeight: 80}}
                                    />
                                </div>
                            </div>
                        )}
                ...
                <div className="action-buttons">
                    {!isEditing ? (
                        <div className="button-group" style={{display: 'flex', justifyContent: 'center', gap: 15, marginTop: 30}}>
                            ...
                            <div className="action-btn logout-btn"
                        ...
                        <div className="button-group" style={{display: 'flex', justifyContent: 'center', gap: 15, marginTop: 30}}>
                            ...
                            <div className="action-btn cancel-btn"
