...
function SyncAdInfo() {
  ...
  useEffect(() => {
    ...
    const axiosConfig = { // axiosConfig도 useEffect 내부에서 정의
      headers: {
    ...
    const fetchAdAccounts = async () => {
      try {
        const response = await axios.get('http://localhost:8080/meta/adaccounts', axiosConfig);
  ...
  const handleSyncAds = async () => {
    ...
    try {
      ...
      setMessage('❌ 광고 정보 동기화 실패: ' + (error.response?.data || error.message));
    }
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      ...
      <main style={{ flexGrow: 1 }}>
        <div style={{ 
          ...
          <h2 style={{textAlign: 'center', marginBottom: '20px', color: '#ffffffff'}}>📊 광고 정보 동기화</h2>

          <div style={{ marginBottom: '20px', textAlign: 'center' }}>
            <label style={{fontWeight: 'bold', color: '#ffffff'}}>📌 광고 계정 선택:</label>
            <select
              value={selectedAccount}
              onChange={(e) => setSelectedAccount(e.target.value)}
              style={{ marginLeft: '10px', padding: '8px', borderRadius: '5px', border: '1px solid #ccc', fontSize: '14px' }}
            >
              <option value="">-- 선택 --</option>
              {adAccounts.map((acc) => (
                <option key={`${acc.accountId}_${acc.pageId}`} value={`${acc.accountId},${acc.pageId}`}>
          ...
          {message && (
            <p style={{ marginTop: '15px', textAlign: 'center', color: message.startsWith('✅') ? '#4CAF50' : '#ff6347' }}>{message}</p>
          )}
        </div>
      </main>
